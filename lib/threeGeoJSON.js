...
export function createGeometryArray(json) {
  let geometry_array = [];
...
export function needsInterpolation(point2, point1) {
...
export function interpolatePoints(interpolation_array) {
...
export function createCoordinateArray(feature) {
...
/**
 * Returns an Object3D that is the spherical version of given coordinates for Globe
 * @param json List of countries with coordinates (/ne_110m_land.json) 
 * @param radius Radius of the Globe
 * @param materialOptions An object contains material values e.g: color, linewidth, fog etc
 * @returns 
 */
export function drawThreeGeo(json, radius, materialOptions) {
  ...
  const y_values = [];
  const z_values = [];
  const json_geom = createGeometryArray(json); // Array containing countries' coords(each element is a country with Polygon or Multipolygon type)
...
/**
 * Takes coords and sphereRadius and converts to latitude and longitude of geoJson
 * @param coords Vector3(x,y,z) 
 * @returns [latitude, longitude]
 */
export const convertToGeoJsonCoords = (coords) => { // Through AI as I know less Math :(
  const lon = Math.atan2(coords.y, coords.x);
  const hyp = Math.sqrt(coords.x * coords.x + coords.y * coords.y);
  const lat = Math.atan2(coords.z, hyp);
...
export const convertToSphereCoords = (coordinates_array, sphere_radius) => {
  const lon = coordinates_array[0];
  const lat = coordinates_array[1];
...
export const drawLine = (x_values, y_values, z_values, options) => {
  const lineGeo = new LineGeometry();
  const verts = [];
  for (let i = 0; i < x_values.length; i++) {
    verts.push(x_values[i], y_values[i], z_values[i]);
