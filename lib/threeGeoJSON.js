...
export function drawThreeGeo(json, radius, materialOptions) {
  ...
  // container.rotation.x = -Math.PI * 0.5; // KLUDGY HACK to fix orientation- Its bad..
  const x_values = [];
  const y_values = [];
  const z_values = [];
 ...
 */
export const convertToGeoJsonCoords = (coords) => {
...
export const createGeometryArray = (json) => {
  let geometry_array = [];
