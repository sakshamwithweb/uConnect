...
const Form = ({ setEvent, showPswrd, setShowPswrd, setSection }) => {
    ...
    const checkUsername = async () => {
        ...
        const isUsernameAcquired = await checkUsernameStatus(username)
        if (isUsernameAcquired) setError("Username has been taken")
        else setError("")
    }
            ...
            <div className='flex flex-col gap-8 w-[50%]'>
                ...
                <form onSubmit={handleSignup} className='flex flex-col gap-4'>
                    <div>
                        ...
                        <Input value={form.username} onChange={(e) => { setForm({ ...form, username: e.target.value }); setUsername(e.target.value) }} onSelect={() => setEvent("emailInput")} type="text" id="username" placeholdeer="Username" />
                    </div>
                    <div>
